...
module BlacklabHelper
  ...
  def reformat_query_attributes(query)
    ...
    query.as_json.select{|key,_| ['patt', 'filter', 'group', 'sort', 'order', 'offset', 'number', 'docpid', 'viewgroup', 'sample', 'samplenum', 'sampleseed', 'gap_values_tsv'].include?(key) }.each do |key, value|
      unless value.blank?
