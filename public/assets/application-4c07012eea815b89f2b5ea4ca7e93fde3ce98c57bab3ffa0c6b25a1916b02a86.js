...
var Whitelab = {
	...
	init : function() {
		...
		$(document).on('click', 'button.btn-pagination', function(e) {
			if ($('#search div#result-pane').length > 0)
				Whitelab.search.getQueryResult($(this).data("number"),$(this).data("offset"));
			else if ($('#explore div#result-pane').length > 0)
...
//			Whitelab.search.history.loadQueryList(5,5);
...
CqlColumn.prototype.stringValues = function() {
	...
	var and = ands.join(" & ");
	if (ands.indexOf("[]") > -1 || ands.indexOf("[word=\".*\"]") > -1) {
		if (this.quantifier == null) {
			options.push("[word=\".*\"]");
		} else {
			options.push("[word=\".*\"]"+this.quantifier);
		}
	} else {
		if (ors.length > 0) {
...
CqlColumn.prototype.setQuantifier = function() {
	...
	if (this.quantifier != null) {
		...
		if (this.quantifier === "{1,1}" || this.quantifier === "{1}") {
			this.quantifier = null;
		}
	}
};
...
;
(function() {
  $(document).on('blur', 'input[data-search-view]', function() {
    $('#view').val($(this).data("search-view"));
    $('#query').val($(this).val());
    return Whitelab.debug('view: ' + $('#view').val() + ', query: ' + $('#query').val());
  });
...
//				Whitelab.goToPage(url+'?query='+cql+'&view='+view);
			}
		}
	},
...
Whitelab.tour = {
	...
	setScrollTop : function(current) {
		if (Whitelab.tour.element != undefined) {
			...
			var pos = $(Whitelab.tour.element).offset().top - Whitelab.tour.offset;
			$("body").animate({
		        scrollTop: pos
		    }, 200);
		}
